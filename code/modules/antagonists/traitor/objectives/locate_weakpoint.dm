...
/obj/item/weakpoint_locator/attack_self(mob/living/user, modifiers)
	...
	if(!do_after(user, 30 SECONDS, src, IGNORE_USER_LOC_CHANGE | IGNORE_TARGET_LOC_CHANGE | IGNORE_HELD_ITEM | IGNORE_INCAPACITATED | IGNORE_SLOWDOWNS, extra_checks = CALLBACK(src, PROC_REF(scan_checks), user, user_area, objective), cog_icon = null))
		playsound(user, 'sound/machines/buzz/buzz-sigh.ogg', 30, TRUE)
		return
