...
/obj/item/organ/brain/cybernetic/ai/proc/is_sufficiently_augmented()
	...
	for(var/obj/item/organ/organ as anything in carb_owner.organs)
		if(organ.organ_flags & ORGAN_EXTERNAL)
			continue
		if(!IS_ROBOTIC_ORGAN(organ) && !istype(organ, /obj/item/organ/tongue) && !istype(organ, /obj/item/organ/appendix)) //tongues and appendixes are not in the exosuit fab and nobody is going to bother to find them so
			return FALSE
