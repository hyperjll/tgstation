...
/mob/living/carbon/human/updateappearance(icon_update = TRUE, mutcolor_update = FALSE, mutations_overlay_update = FALSE)
	...
	if(dna.features["fox_tail"])
		dna.features["fox_tail"] = SSaccessories.tails_list_fox[deconstruct_block(get_uni_feature_block(features, DNA_FOX_TAIL_BLOCK), length(SSaccessories.tails_list_fox))]
