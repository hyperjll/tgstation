...
/proc/start_unequip_mob(obj/item/item, mob/source, mob/user, strip_delay, hidden = FALSE)
	...
	if (!do_after(user, delay, source, interaction_key = REF(item), cog_icon = hidden ? null : 'icons/effects/progressbar.dmi'))
		return FALSE
...
/proc/finish_unequip_mob(obj/item/item, mob/source, mob/user)
	...
	if(HAS_TRAIT(user, TRAIT_THIEF))
		user.log_message("has STEALTHILY stripped [key_name(source)] of [item].", LOG_ATTACK, color="red")
		source.log_message("has been STEALTHILY stripped of [item] by [key_name(user)].", LOG_VICTIM, color="orange", log_globally=FALSE)
	else
