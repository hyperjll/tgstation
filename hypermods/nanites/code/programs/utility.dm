...
/datum/nanite_program/construct_tool
	...
	desc = "The nanites gather into the palm of the host's hand and form a specific tool such as a crowbar or wrench. \
			Items disappear 30 seconds after they're constructed."
	unique = FALSE
	can_trigger = TRUE
	trigger_cost = 40
	trigger_cooldown = 30
...
/datum/nanite_program/convert_nanites/on_trigger(comm_message)
	...
	var/obj/item/new_sheet
	switch(scanned_nanites.get_value())
		if("One Iron Sheet")
			new_sheet = new /obj/item/stack/sheet/iron(host_mob.loc)
		if("One Glass Sheet")
			new_sheet = new /obj/item/stack/sheet/glass(host_mob.loc)
		if("One Plastic Sheet")
			new_sheet = new /obj/item/stack/sheet/plastic(host_mob.loc)
		if("One Plasma Sheet")
			new_sheet = new /obj/item/stack/sheet/mineral/plasma(host_mob.loc)
		if("One Wood Sheet")
			new_sheet = new /obj/item/stack/sheet/mineral/wood(host_mob.loc)
	if(!host_mob.put_in_active_hand(new_sheet))
		new_sheet.forceMove(host_mob.drop_location())
...
/datum/nanite_program/construct_tool_adv/on_trigger(comm_message)
	...
	switch(scanned_nanites.get_value())
		if("Multitool")
			new_tool = new /obj/item/multitool(host_mob.loc)
		if("Jaws of Life")
			new_tool = new /obj/item/crowbar/power(host_mob.loc)
		if("RPD")
			new_tool = new /obj/item/pipe_dispenser(host_mob.loc)
		if("Fire Extinguisher")
			new_tool = new /obj/item/extinguisher(host_mob.loc)
		if("Crew Pinpointer")
			new_tool = new /obj/item/pinpointer/crew(host_mob.loc)
		if("Holofan Projector")
			new_tool = new /obj/item/holosign_creator/atmos(host_mob.loc)
	new_tool.desc += " Upon closer inspection, the material is moving ever-so-slightly."
	if(!host_mob.put_in_active_hand(new_tool))
		new_tool.forceMove(host_mob.drop_location())
	addtimer(CALLBACK(src, .proc/destroy_nanite_item_1, new_tool), 20 SECONDS)

/datum/nanite_program/construct_tool_adv/proc/destroy_nanite_item_1(obj/item/nanite_tool)
	nanite_tool.visible_message(span_danger("[nanite_tool] is beginning to destabilize..."), span_userdanger("[nanite_tool] is destabilizing..."))
	addtimer(CALLBACK(src, .proc/destroy_nanite_item_2, nanite_tool), 10 SECONDS)

/datum/nanite_program/construct_tool_adv/proc/destroy_nanite_item_2(obj/item/nanite_tool)
	nanite_tool.visible_message(span_danger("[nanite_tool] crumbles into nothing..."), span_userdanger("[nanite_tool] suddenly vanishes with little fanfare."))
	qdel(nanite_tool)
...
/datum/nanite_program/construct_tool_super/on_trigger(comm_message)
	...
	switch(scanned_nanites.get_value())
		if("Holotool")
			new_tool = new /obj/item/holotool(host_mob.loc)
	new_tool.desc += " Upon closer inspection, the material is moving ever-so-slightly."
	if(!host_mob.put_in_active_hand(new_tool))
		new_tool.forceMove(host_mob.drop_location())
	addtimer(CALLBACK(src, .proc/destroy_nanite_item_1, new_tool), 20 SECONDS)

/datum/nanite_program/construct_tool_super/proc/destroy_nanite_item_1(obj/item/nanite_tool)
	nanite_tool.visible_message(span_danger("[nanite_tool] is beginning to destabilize..."), span_userdanger("[nanite_tool] is destabilizing..."))
	addtimer(CALLBACK(src, .proc/destroy_nanite_item_2, nanite_tool), 10 SECONDS)

/datum/nanite_program/construct_tool_super/proc/destroy_nanite_item_2(obj/item/nanite_tool)
	nanite_tool.visible_message(span_danger("[nanite_tool] crumbles into nothing..."), span_userdanger("[nanite_tool] suddenly vanishes with little fanfare."))
	qdel(nanite_tool)
...
/datum/nanite_program/monitoring/active_effect()
	var/mob/living/tracked_living = host_mob

	if(!(tracked_living in GLOB.suit_sensors_list)) // If you aren't in the suit sensor list because you have no uniform, it'll add you.
		GLOB.suit_sensors_list |= tracked_living
