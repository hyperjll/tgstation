...
/datum/action/cooldown/spell/pointed/projectile/acid_spit
	...
	sound = 'hypermods/sound/effects/spitacid.ogg'
...
/datum/action/cooldown/spell/pointed/projectile/acid_spit/proc/open_closet(mob/living/carbon/human/owner, obj/structure/closet/C)
	if(C && owner.loc == C)
		...
		C.broken = TRUE
		C.open()
		to_chat(owner, span_warning("You open the container restraining you!"))
...
/obj/projectile/bullet/acid_spit/on_hit(atom/target, blocked = FALSE, pierce_hit)
	...
	if(!isopenturf(target))
		target.acid_act(50 * multiplier, 15)

	if(iscarbon(target))
		target.acid_act(8 * multiplier, 15)
...
/datum/action/cooldown/spell/pointed/projectile/acid_spit/syndicate
	...
	active_msg = "You focus your acid spit!"
	deactive_msg = "You relax."
	projectile_type = /obj/projectile/bullet/acid_spit/syndicate
	acid_multiplier = 1.5
