...
/datum/action/cooldown/spell/pointed/glove_zap/proc/target_tesla_zap(atom/source, atom/target, power, zap_flags = ZAP_DEFAULT_FLAGS, max_damage = INFINITY)
	...
	var/dust_power = 10 GIGA WATTS
	var/heavy_emp_threshold = 1 MEGA WATTS
		...
		if(do_after(owner, 3 SECONDS, target, IGNORE_TARGET_LOC_CHANGE))
			...
			if(get_dist(owner, target) >= cast_range)
				...
				if(surplus <= heavy_zap) //plays a separate sound at 2 MW excess
					playsound(target, 'sound/effects/magic/lightningshock.ogg', 50, TRUE, -1)
				else
					playsound(target, 'sound/effects/magic/lightningbolt.ogg', 50, TRUE, -1)
...
/datum/action/cooldown/spell/pointed/glove_zap/InterceptClickOn(mob/living/clicker, params, atom/target)
	. = ..()
	glove_nerd_zap(target, clicker)
	return TRUE
