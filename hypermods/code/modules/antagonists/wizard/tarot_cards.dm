/obj/item/tarot_generator
	...
	icon = 'hypermods/icons/obj/playing_cards.dmi'
...
/obj/item/tarot_generator/examine(mob/user)
	. = ..()
	. += span_hierophant("Ctrl-Click to destroy all cards it has produced.")
...
/obj/item/tarot_generator/item_ctrl_click(mob/user)
...
/obj/item/blank_tarot_card
	...
	/// If a person can choose which card they want from a blank.
...
/obj/item/magic_tarot_card/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(isliving(hit_atom) && our_tarot)
		pre_activate(hit_atom, throwingdatum?.get_thrower())
		return
	qdel(src)
...
/obj/item/magic_tarot_card/proc/dust_card()
...
/obj/item/magic_tarot_card/proc/pre_activate(mob/user, atom/movable/thrower)
	...
	if(ismob(thrower) && our_tarot)
		thrower.log_message("[thrower] has activated [our_tarot.name] on [user]", LOG_ATTACK)
...
/datum/tarot/the_fool/activate(mob/living/target)
	var/atom/destination = target.mind?.assigned_role.get_latejoin_spawn_point()
	if(!destination)
		return
	if(do_teleport(target, destination, asoundout = 'sound/effects/phasein.ogg', channel = TELEPORT_CHANNEL_BLUESPACE))
...
/datum/tarot/the_magician
	...
	extended_desc = "makes the user feel extraordinarily fearless for a couple of minutes."
	card_icon = "the_magician"

/datum/tarot/the_magician/activate(mob/living/target)
	target.apply_status_effect(/datum/status_effect/the_magician)
...
/obj/effect/abstract/bubblegum_rend_helper/proc/rend(mob/living/owner, damage)
	...
	for(var/i in 1 to 3)
		...
		playsound(second_turf, 'sound/effects/magic/demon_attack1.ogg', 100, TRUE, -1)
...
/datum/tarot/the_empress
	...
	extended_desc = "gives the user a temporary boost of speed and healing."
...
/datum/tarot/the_empress/activate(mob/living/target)
	if(ishuman(target))
		...
		H.reagents.add_reagent(/datum/reagent/inverse/baldium, 5)
...
/datum/tarot/the_emperor/activate(mob/living/target)
	...
	if(!length(L))
		...
		return
	if(do_teleport(target, pick(L), asoundout = 'sound/effects/phasein.ogg', channel = TELEPORT_CHANNEL_BLUESPACE))
		to_chat(target, span_userdanger("You are abruptly pulled through space!"))

/datum/tarot/the_hierophant
	name = "V - The Hierophant"
	desc = "Two prayers for the lost."
	extended_desc = "enchants the user/target with magic that's strong enough to negate three attacks."
	card_icon = "the_hierophant"

/datum/tarot/the_hierophant/activate(mob/living/target)
	target.apply_status_effect(/datum/status_effect/force_shield/hierophant)
...
/datum/tarot/the_lovers/activate(mob/living/target)
	if(ishuman(target))
		var/mob/living/carbon/human/H = target
		H.adjust_brute_loss(-40, updating_health = TRUE)
		H.adjust_fire_loss(-40, updating_health = TRUE)
		H.adjust_blood_volume(100, maximum = BLOOD_VOLUME_NORMAL)
	else
		target.adjust_brute_loss(-40, updating_health = TRUE)
		target.adjust_fire_loss(-40, updating_health = TRUE)
	target.adjust_oxy_loss(-40, updating_health = TRUE)
	target.adjust_tox_loss(-40, updating_health = TRUE, forced = TRUE)

/datum/tarot/the_chariot
	name = "VII - The Chariot"
	desc = "May nothing stand before you."
	extended_desc = "imbues the user with immense power and speed, rendering them practically immortal for 16 seconds, at the cost of being unable to harm another living thing."
	card_icon = "the_chariot"

/datum/tarot/the_chariot/activate(mob/living/target)
	playsound(target, 'hypermods/sound/effects/chariot.ogg', 35, FALSE)
	target.apply_status_effect(/datum/status_effect/the_chariot)
...
/datum/tarot/justice/activate(mob/living/target)
	...
	var/list/health_item_list = list(
		/obj/item/storage/medkit/emergency,
		/obj/item/storage/medkit/regular,
		/obj/item/storage/medkit/advanced
	)

	var/list/bomb_item_list = list(
	...
	var/health_pick = pick(health_item_list)
	var/bomb_item = pick(bomb_item_list)

	new health_pick(target_turf)
	new bomb_item(target_turf)
	new /obj/item/magic_key(target_turf)
...
/datum/tarot/the_hermit/activate(mob/living/target)
	...
	for(var/obj/machinery/vending/candidate in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/vending))
	...
	if(!length(viable_vendors))
		...
		return
	if(do_teleport(target, pick(viable_vendors), asoundout = 'sound/effects/phasein.ogg', channel = TELEPORT_CHANNEL_BLUESPACE))
		to_chat(target, span_userdanger("You are abruptly pulled through space!"))
...
/datum/tarot/wheel_of_fortune/activate(mob/living/target)
	var/list/static/bad_vendors = typesof(/obj/machinery/vending/liberationstation)\
								+ typesof(/obj/machinery/vending/toyliberationstation)\
								+ typesof(/obj/machinery/vending/wallhypo)\
								+ typesof(/obj/machinery/vending/wallgene)\
								+ typesof(/obj/machinery/vending/wallmed) // Future proofing in case we add more subtypes of disallowed vendors
	var/turf/target_turf = get_turf(target)
	var/vendorpath = pick(subtypesof(/obj/machinery/vending) - bad_vendors)
...
/datum/tarot/strength/activate(mob/living/target)
	target.apply_status_effect(/datum/status_effect/tarot_strength)
...
/datum/tarot/the_hanged_man/activate(mob/living/target)
	target.AddElement(/datum/element/forced_gravity, 0)
	addtimer(CALLBACK(target, TYPE_PROC_REF(/datum/, _RemoveElement), list(/datum/element/forced_gravity, 0)), 1 MINUTES)
...
/datum/tarot/temperance/activate(mob/living/target)
	...
	var/list/bad_organs = list(
		target.get_organ_by_type(/obj/item/organ/body_egg),
		target.get_organ_by_type(/obj/item/organ/legion_tumour),
		target.get_organ_by_type(/obj/item/organ/zombie_infection),
		target.get_organ_by_type(/obj/item/organ/empowered_borer_egg), //CORTICAL_BORERS
	)
	...
	for(var/o in bad_organs)
		...
		O.Remove(target)
		H.vomit(VOMIT_CATEGORY_DEFAULT, lost_nutrition = 0)
		O.forceMove(get_turf(target))

	H.reagents.add_reagent(/datum/reagent/medicine/mutadone, 3)

	H.adjust_organ_loss(ORGAN_SLOT_BRAIN, -150)
	H.adjust_organ_loss(ORGAN_SLOT_HEART, -25)
	H.adjust_organ_loss(ORGAN_SLOT_LUNGS, -25)
	H.adjust_organ_loss(ORGAN_SLOT_LIVER, -25)
	H.adjust_organ_loss(ORGAN_SLOT_STOMACH, -25)
	H.adjust_organ_loss(ORGAN_SLOT_EYES, -25)
	H.adjust_organ_loss(ORGAN_SLOT_EARS, -25)
	H.adjust_organ_loss(ORGAN_SLOT_APPENDIX, -25)
...
/datum/tarot/the_devil
	...
	extended_desc = "conjures an infernal contract straight from the devil's desk, there's no telling what contract you might get."
	card_icon = "the_devil"

/datum/tarot/the_devil/activate(mob/living/target)
	var/list/types = subtypesof(/obj/item/soul_contract/)
	for(var/i in 1 to 1)
		var/type = pick(types)
		new type(get_turf(target))
...
/datum/tarot/the_tower/activate(mob/living/target)
	...
	bakoom.arm_grenade(target)

/// I'm sorry matt, this is very funny.
/datum/tarot/the_stars
	name = "XVII - The Stars"
	desc = "May you find what you desire."
	extended_desc = "teleports the user to the station's evidence room, and opens a single locker within where there shall be guarenteed contraband."
	card_icon = "the_stars"
...
/datum/tarot/the_stars/activate(mob/living/target)
	...
	for(var/obj/structure/closet/C in shuffle(view(9, target)))
		if(istype(C, /obj/structure/closet/secure_closet))
			...
			SC.locked = FALSE
		C.open()
		new /obj/effect/spawner/random/contraband(get_turf(C))
...
/datum/tarot/the_moon/activate(mob/living/target)
	...
	if(length(funny_ruin_list))
		do_teleport(target, get_turf(pick(funny_ruin_list)), asoundout = 'sound/effects/phasein.ogg', channel = TELEPORT_CHANNEL_BLUESPACE)
		return
...
/datum/tarot/the_moon/proc/teleport(mob/living/target, turf/teleport_location)
	teleport_location.ChangeTurf(/turf/open/floor/plating) //we give them plating so they are not trapped in a wall or fall into lava/chasm, and a pickaxe to avoid being trapped in a wall
...
/datum/tarot/judgement/activate(mob/living/target)
	notify_ghosts("[target] has used a judgment card. Judge them. Or not, up to you.", source = target, notify_flags = NOTIFY_CATEGORY_NOFLASH, header = "Judgement")
...
/datum/tarot/the_world/activate(mob/living/target)
	do_smoke(10, target, get_turf(target), smoke_type = /datum/effect_system/fluid_spread/smoke/bad)
	target.apply_status_effect(/datum/status_effect/temporary_xray/the_world)
	...
	for(var/atom/movable/AM as anything in thrown_atoms)
		...
		if(dist_from_user == 0)
			if(isliving(AM))
				...
				M.adjust_brute_loss(10)
				to_chat(M, span_userdanger("You're slammed into the floor by [name]!"))
				M.log_message("[M] was thrown by [target]'s [name]", LOG_ATTACK)
		else
...
/datum/tarot/reversed/the_high_priestess/activate(mob/living/target)
	...
	target.cause_hallucination(/datum/hallucination/oh_yeah, "II - The High Priestess?", haunt_them = TRUE)
...
/datum/tarot/reversed/the_empress/activate(mob/living/target)
	for(var/mob/living/L in oview(9, target))
		if(L.can_block_magic(antimagic_flags))
			...
			continue
		L.apply_status_effect(/datum/status_effect/the_empress_r)
...
/datum/tarot/reversed/the_emperor/activate(mob/living/target)
	...
	if(!length(L))
		...
		return
	if(do_teleport(target, get_turf(pick(L)), asoundout = 'sound/effects/phasein.ogg', channel = TELEPORT_CHANNEL_BLUESPACE))
		to_chat(target, span_userdanger("You are abruptly pulled through space!"))
...
/datum/tarot/reversed/the_lovers/activate(mob/living/target)
	if(!ishuman(target))
		return
	var/mob/living/carbon/human/H = target
	var/obj/item/bodypart/L = H.get_bodypart(BODY_ZONE_CHEST)

	var/datum/wound/blunt/bone/severe/breakdown = new
	breakdown.apply_wound(L)
	L.receive_damage(20)

	H.adjust_organ_loss(ORGAN_SLOT_HEART, 20)
...
/datum/tarot/reversed/justice/activate(mob/living/target)
	var/list/types = subtypesof(/datum/supply_pack/)
	var/type = pick(types)
	if(!type)
		return
	var/datum/supply_pack/S = new type
	S.generate(get_turf(target))
...
/datum/tarot/reversed/strength/activate(mob/living/target)
	for(var/mob/living/M in oview(9, target))
		M.adjust_hallucinations_up_to(2 MINUTES, 2 MINUTES)
		M.cause_hallucination( \
		...
		M.adjust_organ_loss(ORGAN_SLOT_BRAIN, 30)
...
/datum/tarot/reversed/the_hanged_man/activate(mob/living/target)
	var/obj/structure/cursed_slot_machine/delete_after_use/pull_the_lever_kronk = new /obj/structure/cursed_slot_machine/delete_after_use(get_turf(target))
	if(ishuman(target))
...
/datum/tarot/reversed/temperance/activate(mob/living/target)
	...
	for(var/i in 1 to 5)
		...
		R.add_reagent(get_random_reagent_id(), 10)
		R.trans_to(H, 10, methods = INGEST)
...
/datum/tarot/reversed/the_devil/activate(mob/living/target)
	var/obj/item/grenade/clusterbuster/flashbang/fuck_your_eyes = new(get_turf(target))
	fuck_your_eyes.arm_grenade(target)
...
/datum/tarot/reversed/the_tower/activate(mob/living/target)
	for(var/turf/T in RANGE_TURFS(9, target))
		...
		if(prob(66))
			continue
		T.ChangeTurf(/turf/closed/mineral/random/labormineral)

/datum/tarot/reversed/the_stars
	name = "XVII - The Stars?"
	desc = "May your loss bring fortune."
	extended_desc = "will inflict four random wounds. However, it will reward the user with two additional cards."
	card_icon = "the_stars?"
...
/datum/tarot/reversed/the_stars/activate(mob/living/target) //Heavy clone damage hit, but gain 2 cards. Not teathered to the card producer. Could lead to card stacking, but would require the sun to fix easily
	...
	for(var/i in 1 to 4)
		var/obj/item/bodypart/bone_to_wound = pick(target.get_bodyparts())
		...
		wound_of_choice.apply_wound(bone_to_wound, wound_source = "XVII - The Stars?")
...
/datum/tarot/reversed/the_moon
	...
	extended_desc = "will reveal a single memory of the user or target to everyone neaby."
	card_icon = "the_moon?"
	antimagic_flags = MAGIC_RESISTANCE|MAGIC_RESISTANCE_MIND

/datum/tarot/reversed/the_moon/activate(mob/living/target)
	var/datum/mind/random_memory = pick(target.mind.memories)
	add_memory_in_range(target, 5, random_memory, deuteragonist = target)
...
/datum/tarot/reversed/the_sun/activate(mob/living/target)
	target.apply_status_effect(/datum/status_effect/reversed_sun)
...
/datum/tarot/reversed/the_world/activate(mob/living/target)
	var/list/L = list()
	for(var/turf/T in get_area_turfs(/area/mine/lounge)) //Lavaland is the abyss, but also too hot to send people too. Mining base should be fair!
		if(T.is_blocked_turf())
			continue
		L.Add(T)
