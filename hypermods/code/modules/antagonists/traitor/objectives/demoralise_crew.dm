	...
	switch(action)
		if ("summon_gear")
			...
			for(var/i in 1 to POSTERS_PROVIDED)
				var/obj/item/poster/traitor/added_poster = new /obj/item/poster/traitor/explosive(posterbox)
...
/datum/traitor_objective/demoralise/graffiti/proc/on_rune_complete(atom/spray, obj/effect/decal/cleanable/traitor_rune/drawn_rune)
	...
	RegisterSignal(drawn_rune, COMSIG_SPRAYCAN_DEMORALISING_EVENT, PROC_REF(on_mood_event))
...
/datum/traitor_objective/demoralise/graffiti/ungenerate_objective()
	if (rune)
		...
		UnregisterSignal(rune, COMSIG_SPRAYCAN_DEMORALISING_EVENT)
