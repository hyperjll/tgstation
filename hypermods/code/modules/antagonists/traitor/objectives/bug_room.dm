...
/datum/traitor_objective/bug_room
	...
	var/obj/item/traitor_bug_room/bug
...
/obj/item/traitor_bug_room/Initialize(mapload)
	. = ..()
	ADD_TRAIT(src, TRAIT_EXAMINE_SKIP, INNATE_TRAIT)

/obj/item/traitor_bug_room/examine(mob/user)
	. = ..()
	if(planted_on)
		return
...
/obj/item/traitor_bug_room/interact(mob/user)
	. = ..()
	if(!target_area_type)
		return
...
/obj/item/traitor_bug_room/interact_with_atom(atom/movable/target, mob/user, proximity_flag, click_parameters)
	. = ..()
...
/obj/item/traitor_bug_room/proc/handle_planted_on_deletion()
	planted_on = null

/obj/item/traitor_bug_room/Destroy()
	if(planted_on)
		vis_flags &= ~VIS_INHERIT_PLANE
		planted_on.vis_contents -= src
	return ..()

/obj/item/traitor_bug_room/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change = TRUE)
	. = ..()
	if(planted_on)
		vis_flags &= ~VIS_INHERIT_PLANE
		planted_on.vis_contents -= src
