/datum/antagonist/traitor/infiltrator/invader/forge_traitor_objectives()
	//if(!employer)
	//return
	switch(employer)
		...
		if(INFILTRATOR_FACTION_GORLEX_MARAUDERS)
			...
			var/datum/objective/hijack/hijacker = new
			hijacker.owner = owner
			hijacker.update_explanation_text()
			objectives += hijacker
...
/datum/objective/escape/escape_with_identity/infiltrator/proc/find_sec_target()
	...
	var/list/crew = SSjob.get_all_crew()
	if(sec.len)
		target = pick(sec)
	else if(crew.len) // Just in case there IS NO SECURITY. Happens on low-pop.
		target = pick(crew)
	else
		find_target()
		return
...
/datum/objective/emag_console/check_completion(mob/user, obj/machinery/computer/communications/target)
