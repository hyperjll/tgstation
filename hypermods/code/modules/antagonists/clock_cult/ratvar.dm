...
/obj/ratvar
	...
	icon = 'hypermods/icons/obj/clock_cult/512x512.dmi'
	icon_state = "ratvar"
	anchored = TRUE
	density = FALSE
...
/obj/ratvar/Initialize(mapload)
	...
	sound_to_playing_players('hypermods/sound/effects/ratvar_reveal.ogg', 100)
	...
	if(area)
		var/mutable_appearance/alert_overlay = mutable_appearance('hypermods/icons/obj/clock_cult/clockwork_effects.dmi', "ratvar_alert")
		notify_ghosts(
	...
	if(GLOB.cult_narsie)
		...
		if(get_dist(src, GLOB.cult_narsie) < 5)
			if(next_attack_tick < world.time)
				...
				sound_to_playing_players('hypermods/sound/magic/clockwork/ratvar_attack.ogg',100)
...
/proc/clockcult_final_ending()
	...
	for(var/mob/lit_mob in GLOB.mob_list)
		if(lit_mob.client)
			...
			SEND_SOUND(lit_mob, sound('sound/effects/magic/fireball.ogg'))
