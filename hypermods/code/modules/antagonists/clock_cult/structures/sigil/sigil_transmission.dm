...
/obj/structure/destructible/clockwork/sigil/transmission/apply_effects(atom/movable/apply_to)
	if(ismecha(apply_to))
		...
		for(var/obj/item/stock_parts/power_store/cell/power_cell as anything in human_contents)
			if(!power_cell) // No cell on you?
				continue

			var/obj/item/stock_parts/power_store/cell/my_cell = power_cell.get_cell()

			if(!my_cell) // Can't locate the cell for some reason?
				continue

			if(IS_CLOCK(human))
				if((my_cell.charge < my_cell.maxcharge) && GLOB.clock_power >= POWER_GIVE)
					my_cell.give(my_cell.chargerate)
					GLOB.clock_power -= POWER_GIVE

			else
				if(my_cell.charge > my_cell.chargerate)
					my_cell.give(-my_cell.chargerate)
					GLOB.clock_power += POWER_SIPHON
