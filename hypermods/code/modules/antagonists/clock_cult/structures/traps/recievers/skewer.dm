...
/obj/structure/destructible/clockwork/trap/skewer
	...
	/// Time until an auto-retract
	var/stab_retract_time = 10 SECONDS
...
/obj/structure/destructible/clockwork/trap/skewer/proc/stab()
	...
	if(target_stabbed)
		if(!stab_overlay)
			stab_overlay = mutable_appearance('hypermods/icons/obj/clock_cult/clockwork_objects.dmi', "brass_skewer_pokeybit", layer = ABOVE_MOB_LAYER)
...
/obj/structure/destructible/clockwork/trap/skewer/unbuckle_mob(mob/living/buckled_mob, force, can_fall)
	...
	addtimer(CALLBACK(src, PROC_REF(retract)), stab_retract_time)
...
/obj/structure/destructible/clockwork/trap/skewer/proc/retract()
	...
	visible_message(span_brass("[src] shudders as it resets."))
