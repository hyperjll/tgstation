...
/obj/item/clothing/suit/clockwork/proc/give_empowered_armor()
	set_armor(empowered_armor)
	if(slowdown >= empowered_slowdown)
...
/obj/item/clothing/suit/clockwork/cloak/attack_self(mob/user, modifiers)
	...
	if(shroud_active)
		disable()
	else if(user.has_status_effect(/datum/status_effect/clockwork_tile))
		enable()
	else
...
/obj/item/clothing/suit/clockwork/cloak/equipped(mob/user, slot)
	...
	wearer = user
	if(shroud_active && user.has_status_effect(/datum/status_effect/clockwork_tile))
		enable()
...
/obj/item/clothing/head/helmet/clockwork/proc/give_empowered_armor(empowered)
	set_armor(/datum/armor/helmet_clockwork_empowered)
