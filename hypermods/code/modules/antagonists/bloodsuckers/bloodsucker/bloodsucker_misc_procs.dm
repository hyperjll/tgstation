...
/datum/antagonist/bloodsucker/proc/break_masquerade(mob/admin)
	if(broke_masquerade)
		return
	owner.current.playsound_local(null, 'hypermods/sound/bloodsuckers/lunge_warn.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/bloodsucker/proc/return_vamp_examine(mob/living/viewer)
	...
	if(viewer.mind.has_antag_datum(/datum/antagonist/vassal) in vassals)
		...
		var/returnIcon = "[icon2html('hypermods/icons/bloodsuckers/vampiric.dmi', world, "bloodsucker")]"
		returnString += "\n"
