...
GLOBAL_LIST_INIT(borer_first_name, world.file2list("hypermods/code/modules/antagonists/borers/code/first_borer_names.txt"))
GLOBAL_LIST_INIT(borer_second_name, world.file2list("hypermods/code/modules/antagonists/borers/code/second_borer_names.txt"))
...
/mob/living/basic/cortical_borer
	...
	icon = 'hypermods/code/modules/antagonists/borers/icons/animal.dmi'
...
/mob/living/basic/cortical_borer/handle_environment(datum/gas_mixture/environment, seconds_per_tick, times_fired)
	...
	if(temp_delta < 0) // it is cold here
		if(!on_fire) // do not reduce body temp when on fire
			adjust_bodytemperature(max(max(temp_delta / BODYTEMP_DIVISOR, BODYTEMP_BORER_COOLING_MAX) * seconds_per_tick, temp_delta))
	else // this is a hot place
		adjust_bodytemperature(min(min(temp_delta / BODYTEMP_DIVISOR, BODYTEMP_BORER_HEATING_MAX) * seconds_per_tick, temp_delta))
