/obj/effect/abstract/liquid_turf
	name = "liquid"
	icon = 'hypermods/icons/effects/liquid.dmi'
	icon_state = "water-0"
	...
	alpha = 175
	//For being on fire
	var/light_outer_range = 0 // prob vestigal now?
	light_power = 1
...
/obj/effect/abstract/liquid_turf/Initialize(mapload, datum/liquid_group/group_to_add)
	...
	RegisterSignal(my_turf, COMSIG_LIVING_Z_IMPACT, PROC_REF(mob_fall))
...
/obj/effect/abstract/liquid_turf/Destroy(force)
	UnregisterSignal(my_turf, list(COMSIG_ATOM_ENTERED, COMSIG_LIVING_Z_IMPACT, COMSIG_ATOM_EXAMINE))
	if(!isnull(my_turf))
...
/obj/effect/abstract/liquid_turf/update_overlays()
	...
	if(number != 0)
		. += mutable_appearance('hypermods/icons/effects/liquid_overlays.dmi', "stage[number]_bottom", offset_spokesman = my_turf, plane = ABOVE_GAME_PLANE, layer = ABOVE_MOB_LAYER)
		. += mutable_appearance('hypermods/icons/effects/liquid_overlays.dmi', "stage[number]_top", offset_spokesman = my_turf, plane =GAME_PLANE, layer = GIB_LAYER)
...
/obj/effect/abstract/liquid_turf/proc/ChangeToNewTurf(turf/NewT)
	...
	UnregisterSignal(my_turf, list(COMSIG_ATOM_ENTERED, COMSIG_LIVING_Z_IMPACT))
...
/obj/effect/temp_visual/liquid_splash
	icon = 'hypermods/icons/effects/splash.dmi'
	icon_state = "splash"
	layer = FLY_LAYER
	randomdir = FALSE
