...
/datum/liquid_group/proc/remove_any(obj/effect/abstract/liquid_turf/remover, amount)
	...
	total_reagent_volume = reagents.total_volume
	if(length(members)) // If this isn't zero, do the math. Prevents division by zero in rare cases.
		reagents_per_turf = total_reagent_volume / length(members)
	else
		reagents_per_turf = total_reagent_volume
	expected_turf_height = CEILING(reagents_per_turf, 1) / LIQUID_HEIGHT_DIVISOR
...
/datum/liquid_group/proc/remove_reagent(obj/effect/abstract/liquid_turf/member, datum/reagent/reagent, amount)
...
/datum/liquid_group/proc/spread_liquid(turf/new_turf, turf/source_turf)
	...
	if(QDELETED(new_turf.liquids) && !istype(new_turf, /turf/open/openspace) && !isspaceturf(new_turf) && source_turf.turf_height == new_turf.turf_height) // no space turfs, or oceans turfs, also don't attempt to spread onto a turf that already has liquids wastes processing time
