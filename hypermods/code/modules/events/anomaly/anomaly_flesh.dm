/datum/round_event_control/anomaly/anomaly_flesh
	name = "Anomaly: Flesh"
	typepath = /datum/round_event/anomaly/anomaly_flesh

	max_occurrences = 5
	weight = 20
	description = "This anomaly occasionally spawns hostile flesh monsters, and will spawn a large amount of flesh monsters as well as fleshzu when going critical."
	min_wizard_trigger_potency = 1
	max_wizard_trigger_potency = 4

/datum/round_event/anomaly/anomaly_flesh
	start_when = ANOMALY_START_HARMFUL_TIME
	announce_when = ANOMALY_ANNOUNCE_HARMFUL_TIME
	anomaly_path = /obj/effect/anomaly/flesh

/datum/round_event/anomaly/anomaly_flesh/announce(fake)
	if(isnull(impact_area))
		impact_area = placer.findValidArea()
	priority_announce("Flesh anomaly detected on [ANOMALY_ANNOUNCE_HARMFUL_TEXT] [impact_area.name].", "Anomaly Alert")
