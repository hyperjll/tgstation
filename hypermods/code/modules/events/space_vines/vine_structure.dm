...
/obj/structure/spacetendons
	name = "tendons"
	desc = "An extremely expansionistic mass of flesh and blood."
	icon = 'hypermods/icons/mob/spacevines.dmi'
	icon_state = "Light1"
	anchored = TRUE
	density = FALSE
	...
	var/datum/spacetendons_controller/master
...
/obj/structure/spacetendons/Initialize(mapload)
	. = ..()
...
/obj/structure/spacetendons/examine(mob/user)
	. = ..()
	if(!length(mutations))
		. += "This vine has no mutations."
		return
	var/text = "This vine has the following mutations:\n"
	for(var/datum/spacetendon_mutation/mutation as anything in mutations)
...
/obj/structure/spacetendons/Destroy()
	for(var/datum/spacetendon_mutation/mutation in mutations)
...
/obj/structure/spacetendons/proc/on_chem_effect(datum/reagent/chem)
	var/override = FALSE
	for(var/datum/spacetendon_mutation/mutation in mutations)
...
/obj/structure/spacetendons/proc/eat(mob/eater)
	var/override = FALSE
	for(var/datum/spacetendon_mutation/mutation in mutations)
...
/obj/structure/spacetendons/attacked_by(obj/item/item, mob/living/user)
...
/obj/structure/spacetendons/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	switch(damage_type)
		if(BRUTE)
			if(damage_amount)
...
/obj/structure/spacetendons/proc/on_entered(datum/source, atom/movable/movable)
	SIGNAL_HANDLER
	if(!isliving(movable))
		return
	for(var/datum/spacetendon_mutation/mutation in mutations)
...
/obj/structure/spacetendons/attack_hand(mob/user, list/modifiers)
	for(var/datum/spacetendon_mutation/mutation in mutations)
		mutation.on_hit(src, user)
	user_unbuckle_mob(user, user)
	return ..()

/obj/structure/spacetendons/attack_paw(mob/living/user, list/modifiers)
	for(var/datum/spacetendon_mutation/mutation in mutations)
		mutation.on_hit(src, user)
	user_unbuckle_mob(user,user)

/obj/structure/spacetendons/attack_alien(mob/living/user, list/modifiers)
	eat(user)

/// Updates the icon as the space vine grows
/obj/structure/spacetendons/proc/grow()
	if(!growth_stage)
...
/obj/structure/spacetendons/proc/entangle_mob()
...
/obj/structure/spacetendons/proc/entangle(mob/living/victim)
...
/obj/structure/spacetendons/proc/spread()
	...
	if(islava(stepturf) && !HAS_TRAIT(stepturf, TRAIT_LAVA_STOPPED))
		return
	var/obj/structure/spacetendons/spot_taken = locate() in stepturf
	var/datum/spacetendon_mutation/vine_eating/eating = locate() in mutations
	...
	var/obj/structure/spacetendons/spawning_vine = master.spawn_spacevine_piece(stepturf, src)
...
/obj/structure/spacetendons/ex_act(severity, target)
	var/index
	for(var/datum/spacetendon_mutation/mutation in mutations)
...
/obj/structure/spacetendons/should_atmos_process(datum/gas_mixture/air, exposed_temperature)
...
/obj/structure/spacetendons/atmos_expose(datum/gas_mixture/air, exposed_temperature)
	for(var/datum/spacetendon_mutation/mutation in mutations)
...
/obj/structure/spacetendons/CanAllowThrough(atom/movable/mover, border_dir)
	. = ..()
