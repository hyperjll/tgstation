...
GLOBAL_LIST_INIT(tendon_mutations_list, init_tendon_mutation_list())

/proc/init_tendon_mutation_list()
	var/list/mutation_list = list()
	init_subtypes(/datum/spacetendons_controller/, mutation_list)
	for(var/datum/spacetendon_mutation/mutation as anything in mutation_list)
		mutation_list[mutation] = IDEAL_MAX_SEVERITY - mutation.severity // the ideal maximum potency is used for weighting
	return mutation_list

/datum/spacetendons_controller
	///Canonical list of all the vines we "own"
	var/list/obj/structure/spacetendons/vines
	...
	var/list/obj/structure/spacetendons/queue_end
...
/datum/spacetendons_controller/New(turf/location, list/muts, potency, production, datum/round_event/event = null)
	vines = list()
	growth_queue = list()
	queue_end = list()
	var/obj/structure/spacetendons/vine = spawn_spacevine_piece(location, null, muts)
	if(event)
...
/datum/spacetendons_controller/vv_get_dropdown()
	. = ..()
	VV_DROPDOWN_OPTION(VV_HK_SPACEVINE_PURGE, "Delete Vines")

/datum/spacetendons_controller/vv_do_topic(href_list)
	. = ..()
...
/datum/spacetendons_controller/proc/DeleteVines() //this is kill
	QDEL_LIST(vines) //this will also qdel us

/datum/spacetendons_controller/Destroy()
...
/datum/spacetendons_controller/proc/spawn_spacevine_piece(turf/location, obj/structure/spacetendons/parent, list/muts)
	var/obj/structure/spacetendons/vine = new(location)
	growth_queue += vine
	vines += vine
	vine.master = src
	for(var/mutation_type in muts)
		for(var/datum/spacetendon_mutation/mutation in GLOB.tendon_mutations_list)
	...
	if(parent)
		...
		if(prob(mutativeness))
			var/datum/spacetendon_mutation/random_mutate = pick_weight(GLOB.tendon_mutations_list - vine.mutations)
			if(!isnull(random_mutate)) //If this vine has every single mutation don't attempt to add a null mutation.
				...
				for(var/datum/spacetendon_mutation/mutation as anything in vine.mutations)
					total_severity += mutation.severity
				if(total_severity <= max_mutation_severity)
					random_mutate.add_mutation_to_vinepiece(vine)

	for(var/datum/spacetendon_mutation/mutation in vine.mutations)
		mutation.on_birth(vine)
	location.Entered(vine, null)
	return vine

/datum/spacetendons_controller/proc/VineDestroyed(obj/structure/spacetendons/vine)
	vine.master = null
	vines -= vine
	growth_queue -= vine
	queue_end -= vine
	if(length(vines))
		return
	var/obj/item/seeds/tendons/seed = new(vine.loc)
...
/datum/spacetendons_controller/process(seconds_per_tick)
	...
	for(var/obj/structure/spacetendons/vine in growth_queue)
		if(!vine.can_spread)
			continue
		growth_queue -= vine
		queue_end += vine
