...
/obj/bitrunning/watcher_hunt_spawner
	...
	var/list/real_spawn_order = list() // i feel the original author's scorn with the original var name for this stupid coding language.
...
/obj/bitrunning/watcher_hunt_spawner/Initialize(mapload)
	. = ..()
	alpha = 0
	real_spawn_order = spawn_order.Copy()
	max_spawned_targets = length(spawn_order)

/obj/bitrunning/watcher_hunt_spawner/proc/spawn_target()
	if(!length(real_spawn_order) && replace_destroyed_targets)
		real_spawn_order = spawn_order.Copy()
	if(length(real_spawn_order))
		if(max_spawned_targets > length(current_targets))
			var/path_to_spawn = pick_n_take(real_spawn_order)
...
/obj/bitrunning/target/barrel
	...
	score = 0
	drop_sounds = list(
		'hypermods/sound/misc/bitrunner/wood_plank_break1.ogg',
		'hypermods/sound/misc/bitrunner/wood_plank_break2.ogg',
		'hypermods/sound/misc/bitrunner/wood_plank_break3.ogg',
		'hypermods/sound/misc/bitrunner/wood_plank_break4.ogg',
	)
...
/obj/machinery/digital_clock/score_card/proc/start_next_round()
	...
	for(var/obj/bitrunning/watcher_hunt_spawner/spawner in found_spawners)
		spawner.real_spawn_order = spawner.spawn_order.Copy()
...
/obj/machinery/digital_clock/score_card/proc/check_score()
	if(current_score >= target_score) // early return?
		deltimer(round_timer)
		for(var/obj/bitrunning/watcher_hunt_spawner/spawner in found_spawners)
			deltimer(spawner.spawn_timer)
			for(var/obj/target in spawner.current_targets)
...
/obj/machinery/digital_clock/score_card/update_time()
	...
	if(!started)
		var/mutable_appearance/click_me_point = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "click_me")
	...
	for(var/i in 1 to 6)
		...
		var/mutable_appearance/current_number = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', number_to_use)
		var/mutable_appearance/current_number_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', number_to_use, src, alpha = DISPLAY_PIXEL_ALPHA)
		var/mutable_appearance/target_number = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', target_number_to_use)
		var/mutable_appearance/target_number_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', target_number_to_use, src, alpha = DISPLAY_PIXEL_ALPHA)
