...
/datum/looping_sound/annoying_light_hum
	mid_sounds = list('hypermods/sound/misc/bitrunner/light_hum.ogg' = 1)
...
/obj/bitrunning/animatronic_phone/attack_hand(mob/living/user, list/modifiers)
	. = ..()
	if(!started_night) // early returns?
		started_night = TRUE
		current_line = 1
		stop_ringing()
...
/obj/bitrunning/door_button
	...
	icon = 'hypermods/icons/obj/machines/bitrunning.dmi'
...
/obj/machinery/digital_clock/bitrunner_power/update_time()
	...
	var/mutable_appearance/percent_symbol = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "percent")
	var/mutable_appearance/percent_symbol_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "percent", src, alpha = DISPLAY_PIXEL_ALPHA)
	...
	var/mutable_appearance/first_digit = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+[first_digit_string]")
	var/mutable_appearance/first_digit_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+[first_digit_string]", src, alpha = DISPLAY_PIXEL_ALPHA)
	...
	if(my_controller.power_left >= 100)
		...
		var/mutable_appearance/hundredth_digit = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+1")
		var/mutable_appearance/hundredth_digit_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+1", src, alpha = DISPLAY_PIXEL_ALPHA)
		hundredth_digit.pixel_w = -16
		hundredth_digit_e.pixel_w = -16
		return_overlays += hundredth_digit
		return_overlays += hundredth_digit_e
		var/mutable_appearance/tenth_digit = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+[first_digit_string[2]]")
		var/mutable_appearance/tenth_digit_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+[first_digit_string[2]]", src, alpha = DISPLAY_PIXEL_ALPHA)
		...
		first_digit_string = "[my_controller.power_left]"
		var/mutable_appearance/tenth_digit = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+[first_digit_string[1]]")
		var/mutable_appearance/tenth_digit_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "+[first_digit_string[1]]", src, alpha = DISPLAY_PIXEL_ALPHA)
		tenth_digit.pixel_w = -12
		tenth_digit_e.pixel_w = -12
		return_overlays += tenth_digit
		return_overlays += tenth_digit_e
	...
	if(my_controller.active_drains)
		var/mutable_appearance/power_usage = mutable_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "powerbar_[my_controller.active_drains]")
		var/mutable_appearance/power_usage_e = emissive_appearance('hypermods/icons/obj/machines/bitrunning.dmi', "powerbar_[my_controller.active_drains]", src, alpha = DISPLAY_PIXEL_ALPHA)
...
/obj/bitrunning/animatronic_controller/proc/power_outage()
	...
	my_clock.update_icon()
	my_power.update_icon()
	playsound(src, 'hypermods/sound/misc/bitrunner/power_outage.ogg', vol = 100, vary = FALSE)
	QDEL_NULL(annoying_light_hum)
	QDEL_NULL(fan_sound)
	for(var/obj/machinery/light/lightbulb in range(10, src))
		if(lightbulb == left_light || lightbulb == right_light)
...
/obj/bitrunning/animatronic_controller/proc/lightsout_1_roll()
	if(prob(20) || lightsout_1_current >= 4)
		...
		if(prob(1))
			playsound(standard_module, 'hypermods/sound/misc/bitrunner/standard_jingle_easter_egg.ogg', 100, FALSE, use_reverb = TRUE, channel = CHANNEL_JUKEBOX)
		else
			playsound(standard_module, 'hypermods/sound/misc/bitrunner/standard_jingle_normal.ogg', 100, FALSE, use_reverb = TRUE, channel = CHANNEL_JUKEBOX)
...
/obj/bitrunning/animatronic_controller/proc/lightout_2_roll()
	if(prob(20) || lightsout_2_current >= 4) // early returns?
		var/obj/bitrunning/animatronic/standard_module = locate(/obj/bitrunning/animatronic/standard) in animatronics
...
/obj/bitrunning/animatronic_controller/proc/you_failed(obj/bitrunning/animatronic/killer_robot)
	...
	for(var/mob/markiplier in range(3, get_turf(src)))
		if(markiplier.client)
			...
			markiplier.playsound_local(get_turf(src), 'hypermods/sound/misc/bitrunner/robot_scream.ogg', 100, FALSE)
...
/obj/bitrunning/animatronic_controller/proc/delete_jumpscare(mob/markiplier, image/jumpscare)
	markiplier?.client?.images -= jumpscare
	qdel(jumpscare)
	markiplier.playsound_local(get_turf(src), 'hypermods/sound/misc/bitrunner/jumpscare_static.ogg', 100, FALSE)
