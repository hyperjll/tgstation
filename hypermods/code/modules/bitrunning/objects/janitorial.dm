GLOBAL_LIST_INIT(janitorial_reject_type_to_speech, list(
	...
	/obj/effect/decal/cleanable/blood/oil = "Oil splatter",
	/obj/effect/decal/cleanable/blood/xeno = "Acidic blood splatter",
...
/obj/machinery/janitorial_scanner/proc/scan_area()
	for(var/turf/floor in our_room)
		if(!isopenturf(floor))
			continue
		for(var/atom/possible_disqualifier as anything in floor)
...
/obj/machinery/janitorial_submit/attack_hand(mob/living/user, list/modifiers)
	...
	for(var/obj/machinery/janitorial_scanner/scanner in GLOB.janitorial_scanners)
		if(unique_id == scanner.unique_id) // early continue if they aren't the same?
...
/obj/effect/bitrunner_exit_portal/attack_hand(mob/living/user, list/modifiers)
	. = ..()
	SEND_SIGNAL(user, COMSIG_BITRUNNER_ALERT_SEVER) // Register and use COMSIG_LIVING_UNARMED_ATTACK instead?
