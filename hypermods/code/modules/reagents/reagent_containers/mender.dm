...
/obj/item/reagent_containers/applicator/patch/mender
	name = "auto-mender"
...
/obj/item/reagent_containers/applicator/patch/mender/examine(mob/user)
	. = ..()
	. += "It has [reagents.total_volume] units left."

/obj/item/reagent_containers/applicator/patch/mender/update_overlays()
	. = ..()
...
/obj/item/reagent_containers/applicator/patch/mender/emag_act(mob/user, obj/item/card/emag/E)
	if(user && E && !tampered)
...
/obj/item/reagent_containers/applicator/patch/mender/on_consumption(mob/consumer, mob/giver)
	if(reagents.total_volume)
		reagents.trans_to(consumer, use_volume, transferred_by = giver, methods = apply_method)
	playsound(src, pick(sfx), 50, 1)
	//qdel(src)

/obj/item/reagent_containers/applicator/patch/mender/interact_with_atom(atom/target, mob/living/user, list/modifiers)
	if (ismob(target))
		var/mob/target_mob = target
		if(!canconsume(target_mob, user))
			return ITEM_INTERACT_BLOCKING
		if(target_mob == user)
			target_mob.visible_message(span_notice("[user] attempts to use [src] on [target_mob]."))
			if(self_delay)
				if(!do_after(user, self_delay, target_mob))
					return ITEM_INTERACT_BLOCKING
			to_chat(target_mob, span_notice("You apply [apply_method] units of [src] to [target_mob]."))
		else
			target_mob.visible_message(span_danger("[user] attempts to force [target_mob] to use [src]."), \
								span_userdanger("[user] attempts to force you to use [src]."))
			if(!do_after(user, CHEM_INTERACT_DELAY(3 SECONDS, user), target_mob))
				return ITEM_INTERACT_BLOCKING
			target_mob.visible_message(span_danger("[user] forces [target_mob] to use [src]."), \
								span_userdanger("[user] forces you to use [src]."))
	...
	if(target.is_drainable() && !target.reagents.total_volume)
		to_chat(user, span_warning("[target] is empty! There's nothing to transfer [src] into."))
		return ITEM_INTERACT_BLOCKING
	...
	user.visible_message(span_warning("[user] slips something into [target]!"), span_notice("You add [src] in [target]."), null, 2)
	reagents.trans_to(target, use_volume, transferred_by = user)
	playsound(src, pick(sfx), 50, 1)
	//qdel(src)
	return ITEM_INTERACT_SUCCESS

/obj/item/reagent_containers/applicator/patch/mender/brute
	name = "auto-mender (brute)"
	list_reagents = list(/datum/reagent/medicine/styptic_powder = 200)

/obj/item/reagent_containers/applicator/patch/mender/burn
