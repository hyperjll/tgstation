...
/obj/item/slimecross/crystalized
	...
	desc = "It's crystalline structure seems to house some kind of anomalous energy."
	effect = "crystalline"
	icon = 'hypermods/icons/obj/science/slimecrossing.dmi'
	icon_state = "crystalline"
	/// What kind of crystal we spawn
	var/obj/structure/slime_crystal/crystal_type
	/// How far away each crystal has to be from one another.
	var/min_crystal_range = 6
	/// How long it takes for us to use an extract
	var/extract_use_time = 15 SECONDS

/obj/item/slimecross/crystalized/attack_self(mob/user)
	. = ..()
	var/obj/structure/slime_crystal/C = locate() in range(min_crystal_range,get_turf(user))
	if(C)
		to_chat(user,span_notice("You can't build crystals that close to each other!"))
		return
	var/user_turf = get_turf(user)
	if(!do_after(user, extract_use_time, user_turf, timed_action_flags = IGNORE_HELD_ITEM))
		return
...
/obj/item/slimecross/crystalized/sepia
	...
	colour = "sepia"
	effect_desc = "Everything in the area is put in pseudo-stasis. Does not incapacitate."
