...
/obj/structure/slime_crystal
	...
	/// How far away can our effects reach?
	var/effect_range = 4
...
/obj/structure/slime_crystal/process()
	if(!uses_process)
		return PROCESS_KILL
	var/list/current_mobs = view_or_range(effect_range, src, range_type)
...
/obj/structure/slime_crystal/proc/view_or_range(dist, source, mode)
	if(mode == "view")
		return view(dist, source)
	return range(dist, source)
...
/obj/structure/slime_crystal/purple/on_mob_effect(mob/living/affected_mob)
	...
	var/mob/living/carbon/carbon_mob = affected_mob
	var/rand_dam_type = rand(1, 4)
	switch(rand_dam_type)
		if(1)
			if(carbon_mob.get_brute_loss() > 0)
				need_mob_update += carbon_mob.adjust_brute_loss(-heal_amt, updating_health = FALSE)
				healed_last_tick = TRUE
		if(2)
			if(carbon_mob.get_fire_loss() > 0)
				need_mob_update += carbon_mob.adjust_fire_loss(-heal_amt, updating_health = FALSE)
				healed_last_tick = TRUE
		if(3)
			if(carbon_mob.get_oxy_loss() > 0)
				need_mob_update += carbon_mob.adjust_oxy_loss(-heal_amt, updating_health = FALSE)
				healed_last_tick = TRUE
		if(4)
			if(carbon_mob.get_tox_loss() > 0)
				need_mob_update += carbon_mob.adjust_tox_loss(-heal_amt, updating_health = FALSE, forced = TRUE) //don't kill jellypeople
				healed_last_tick = TRUE
...
/obj/structure/slime_crystal/blue/process()
	for(var/turf/open/T in view(2, src))
		...
		if(T.blocks_air)
			continue
		T.copy_air(base_mix.copy())
		T.air_update_turf(update = FALSE, remove = FALSE)
...
/obj/structure/slime_crystal/metal/on_mob_effect(mob/living/affected_mob)
	...
	borgo.adjust_fire_loss(-heal_amt)
...
/obj/structure/slime_crystal/yellow/attacked_by(obj/item/I, mob/living/user)
	if(istype(I, /obj/item/stock_parts/power_store))
		var/obj/item/stock_parts/power_store/cell = I
...
/obj/structure/slime_crystal/darkpurple/process()
	...
	if(air.has_gas(/datum/gas/plasma, 15))
...
/obj/structure/slime_crystal/darkpurple/Destroy()
	var/turf/T = get_turf(src)
	T.atmos_spawn_air("plasma=[20];TEMP=[500]")
	return ..()
...
/obj/structure/slime_crystal/darkblue/process()
	var/list/listie = range(effect_range,src)
...
/obj/structure/slime_crystal/sepia
	colour = "sepia"
	effect_desc = "Everything in the area around it is put in a pseudo-stasis. Does not incapacitate."

/obj/structure/slime_crystal/sepia/on_mob_enter(mob/living/affected_mob)
	affected_mob.add_traits(list(TRAIT_NOBREATH, TRAIT_NOCRITDAMAGE, TRAIT_NOSOFTCRIT, TRAIT_NOHARDCRIT, TRAIT_TUMOR_SUPPRESSED), "sepia_crystal")

/obj/structure/slime_crystal/sepia/on_mob_leave(mob/living/affected_mob)
	affected_mob.remove_traits(list(TRAIT_NOBREATH, TRAIT_NOCRITDAMAGE, TRAIT_NOSOFTCRIT, TRAIT_NOHARDCRIT, TRAIT_TUMOR_SUPPRESSED), "sepia_crystal")
...
/obj/structure/slime_crystal/cerulean/process()
	for(var/turf/T in range(effect_range,src))
		if(T.is_blocked_turf() || isspaceturf(T)  || T == get_turf(src) || prob(50))
			continue
		var/obj/structure/cerulean_slime_crystal/CSC = locate() in range(effect_range,T)
		if(CSC)
			continue
...
/obj/structure/slime_crystal/pyrite
	...
	density = FALSE // Just for you, party-goers.
	uses_process = FALSE
	effect_range = 4
...
/obj/structure/slime_crystal/pyrite/proc/change_colour()
	...
	for(var/turf/T in RANGE_TURFS(effect_range,src))
...
/obj/structure/slime_crystal/red
	...
	var/list/reward_item_list = list(
...
/obj/structure/slime_crystal/red/process()
	if(blood_amt == max_blood_amt)
		return
	for(var/obj/effect/decal/cleanable/B in range(effect_range,src))
...
/obj/structure/slime_crystal/red/attack_hand(mob/user)
	...
	playsound(src, 'sound/effects/magic/demon_consume.ogg', 50, 1)

	var/item = pick(reward_item_list)
...
/obj/structure/slime_crystal/red/attacked_by(obj/item/I, mob/living/user)
	...
	if(!istype(I, /obj/item/reagent_containers/cup))
		return ..()
	var/obj/item/reagent_containers/cup/reagent_item = I
	if(!reagent_item.is_refillable() || (reagent_item.reagents.total_volume + 10 > reagent_item.reagents.maximum_volume))
		return ..()
	blood_amt -= 10
	reagent_item.reagents.add_reagent(/datum/reagent/blood, 10)
	to_chat(user, span_notice("You transfer some of stored blood into [I]!"))
...
/obj/structure/slime_crystal/green/on_mob_effect(mob/living/affected_mob)
	...
	human_mob.dna.add_mutation(stored_mutation, "green_crystal")
	...
	human_mob.dna.remove_mutation(mutation, "green_crystal")

/obj/structure/slime_crystal/green/on_mob_leave(mob/living/affected_mob)
	if(!ishuman(affected_mob))
		return
	var/mob/living/carbon/human/human_mob = affected_mob
	human_mob.dna.remove_mutation(stored_mutation, "green_crystal")
...
/obj/structure/slime_crystal/pink/on_mob_enter(mob/living/affected_mob)
	ADD_TRAIT(affected_mob, TRAIT_PACIFISM, "pick_crystal")

/obj/structure/slime_crystal/pink/on_mob_leave(mob/living/affected_mob)
	REMOVE_TRAIT(affected_mob, TRAIT_PACIFISM, "pick_crystal")
...
/obj/structure/slime_crystal/gold/attack_hand(mob/user)
	...
	var/mob/living/basic/chosen_pet = pick(gold_pet_options)
	chosen_pet = new chosen_pet(get_turf(human_mob))
	if(chosen_pet)
		to_chat(user, span_notice("You touch the crystal, and become a small animal!"))
		playsound(src, 'sound/effects/magic/fireball.ogg', 50, 1)
		human_mob.forceMove(chosen_pet)
		human_mob.add_traits(list(TRAIT_GODMODE, TRAIT_BOMBGIBIMMUNE, TRAIT_MUTE), "gold_crystal")
...
/obj/structure/slime_crystal/gold/on_mob_leave(mob/living/affected_mob)
	...
	if(!human_mob)
		return
	human_mob.remove_traits(list(TRAIT_GODMODE, TRAIT_BOMBGIBIMMUNE, TRAIT_MUTE), "gold_crystal")
...
/obj/structure/slime_crystal/oil/process()
	for(var/T in RANGE_TURFS(effect_range,src))
		if(!isopenturf(T))
			continue
...
/obj/structure/slime_crystal/lightpink/attack_ghost(mob/user)
	. = ..()
	var/be_helper = tgui_alert(user, "Become a Lightgeist? (Warning, You can no longer be revived!)", "Unrevivable Ghost Role", list("Yes", "No"))
	if(be_helper == "Yes" && !QDELETED(src) && isobserver(user))
		var/mob/living/basic/lightgeist/W = new /mob/living/basic/lightgeist/slime(get_turf(loc))
		W.key = user.key
...
/obj/structure/slime_crystal/adamantine/on_mob_enter(mob/living/affected_mob)
	...
	var/mob/living/carbon/human/our_human = affected_mob
	our_human.physiology.brute_mod *= 0.5  //Hefty resistance, great for point defence.
...
/obj/structure/slime_crystal/adamantine/on_mob_leave(mob/living/affected_mob)
	if(!ishuman(affected_mob))
		return
	var/mob/living/carbon/human/our_human = affected_mob
	our_human.physiology.brute_mod *= 2
	our_human.physiology.burn_mod *= 2
