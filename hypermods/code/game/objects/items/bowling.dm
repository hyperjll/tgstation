/obj/item/bowling_ball
	...
	icon = 'hypermods/icons/obj/bowling_ball.dmi'
	worn_y_offset = 4
	worn_icon = 'hypermods/icons/mob/clothing/head/hats.dmi'
	righthand_file = 'hypermods/icons/mob/inhands/weapons/balls_righthand.dmi'
	lefthand_file = 'hypermods/icons/mob/inhands/weapons/balls_lefthand.dmi'
	force = 6
...
/obj/item/bowling_ball/throw_at(atom/target, range = 150, speed, mob/thrower, spin, diagonals_first, datum/callback/callback, force, gentle, quickstart)
	...
	playsound(src,'hypermods/sound/effects/bowl.ogg',40,0)
	var/mob/living/carbon/human/user = thrower
	if(user.w_uniform && istype(user.w_uniform, /obj/item/clothing/under/bowling_jersey/real))
		pro_wielded = TRUE
...
/obj/item/bowling_ball/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(pro_wielded || prob(10))
		...
		if(iscarbon(hit_atom) && target_mob.body_position == STANDING_UP && throwingdatum.dist_travelled > 1)
			playsound(src, 'hypermods/sound/effects/bowlhit.ogg', 60, 0)
			target_mob.Knockdown(60)
			target_mob.adjust_brute_loss(15) // + throw damage = 25 hit, ouch!
	return ..()
